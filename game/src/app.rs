    ...
    fn render_game(&self, args: &RenderArgs, c: Context, gl: &mut GlGraphics) {
        ...
        let GameOnly(state) | OverlayMenu(_, state) = &self.gui.active_menu {
            if let GameState::GameState { apple, .. } = &state {
                ...
                {
                    let transform = c.trans(-PLAYER_SIZE / 2.0, -PLAYER_SIZE / 2.0).trans(-GAME_SIZE as f64 * TILE_SIZE, -GAME_SIZE as f64 * TILE_SIZE).transform;
                    let game_area = [0.0, 0.0, GAME_SIZE as f64 * 2.0 * TILE_SIZE, GAME_SIZE as f64 * 2.0 * TILE_SIZE];

                    rectangle(BLUE, game_area, transform, gl);
                }

                if let Some(apple) = apple {
                    let transform = c.trans(-PLAYER_SIZE / 2.0, -PLAYER_SIZE / 2.0).trans(TILE_SIZE * (apple.x as f64), TILE_SIZE * (apple.y as f64)).transform;
    ...
    pub fn update(&mut self, args: UpdateArgs, window: &mut PistonWindow<GlutinWindow>) {
        ...
        match &self.gui.active_menu {
            GameOnly(GameState::GameOver { score }) => {
                widget::Text::new(&format!("Final length: {}", score)).font_size(30).middle_of(self.gui.ids.main_canvas).set(self.gui.ids.menu_title, ui);

                let mut result: Box<dyn FnMut(&mut GUIVisibility) -> ()> = Box::new(|_|{});

                for press in widget::Button::new().label("Start New Game").down_from(self.gui.ids.menu_title,10.0).set(self.gui.ids.editor_button, ui) {
